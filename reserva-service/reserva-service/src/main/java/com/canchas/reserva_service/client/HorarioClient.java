package com.canchas.reserva_service.client;

import com.canchas.reserva_service.client.dto.HorarioDTO;
import org.springframework.cloud.openfeign.FeignClient;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.PutMapping;
import org.springframework.web.bind.annotation.RequestParam;

@FeignClient(name = "horario-service")
public interface HorarioClient {

    @GetMapping("/horarios/{id}")
    HorarioDTO obtenerHorario(@PathVariable Long id);

    @PutMapping("/horarios/{id}/disponibilidad")
    HorarioDTO cambiarDisponibilidad(@PathVariable Long id, @RequestParam boolean disponible);
}